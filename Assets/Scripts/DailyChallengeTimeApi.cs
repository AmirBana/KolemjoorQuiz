...
public class DailyChallengeTimeApi : MonoBehaviour
{
    DateTime dt1;
    ...
    private void Start() {
        if(ObscuredPrefs.GetString("DailyChallengeDate") != "") {
            dt1 = DateTime.Parse(ObscuredPrefs.GetString("DailyChallengeDate"));
    ...
    IEnumerator Counter() {
        while(true) {
            ...
            if(WorldTimeAPI.Instance.IsTimeLoaded) {
                ...
                if(th <= 0 && tm <= 0 && ts <= 0) {
                    ObscuredPrefs.SetString("DailyChallengeDate", "");
